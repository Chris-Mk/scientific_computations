...
def perform_Gaussian_fit(x, y, region_start, region_stop, mu_guess, A_guess=0, sigma_guess=1, left_selection=None,
                         right_selection=None, plotting=1, printing=1):
    # these define a region of interest in our data binning:
    region = (region_start < x) & (x < region_stop)
    # now limit our data by 'slicing' it, limiting it to the region of interest:
    peak_region_bc = x[region]
    ...
    if (left_selection and right_selection):
        ...
        lin_region = ((x > left_selection[0]) & (x < left_selection[1])) | (
                    (x > right_selection[0]) & (x < right_selection[1]))
        lin_bc = x[lin_region]
    ...
    g_final = fit_Gaussian(peak_region_bc, peak_region_counts, region_start, region_stop, mu_guess, A_guess,
                           sigma_guess)
    # Check if fit worked 
    if (g_final == 0):
        return
    ...
    if (plotting):
        ...
        if (left_selection and right_selection):
            plt.step(peak_region_bc, peak_region_counts + line, where='mid', color='cornflowerblue',
                     label='data')  # plotting data
            plt.vlines(left_selection + right_selection, ymin=0,
                       ymax=g_final.A)  # plot support lines around selected points
            plt.plot(peak_region_bc, line + GaussFunc(peak_region_bc, g_final.A, g_final.mu, g_final.sigma),
                     color=color, label='Gaussian fit')  # plot Gaussian
            plt.plot(lin_bc, LineFunc(lin_bc, estimates_lin[0], estimates_lin[1]), color='r', label='linear fit',
                     alpha=0.6)  # plot linear fit
        else:
            plt.step(peak_region_bc, peak_region_counts, where='mid', color='cornflowerblue',
                     label='data')  # plotting data
            plt.plot(peak_region_bc, GaussFunc(peak_region_bc, g_final.A, g_final.mu, g_final.sigma), color=color,
                     label='Gaussian fit')
